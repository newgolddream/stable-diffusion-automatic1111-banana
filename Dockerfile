...
RUN wget -O models/Stable-diffusion/model.safetensors 'https://huggingface.co/XpucT/Deliberate/resolve/main/Deliberate.safetensors'
RUN echo 2
ADD prepare.py .
...
CMD ["python", "server.py", "--xformers", "--lowram", "--no-half", "--no-hashing", "--listen", "--port", "8000"]
